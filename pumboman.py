...
if run:
    ...
    if mode == "📋 단일 유형 조회":
        ...
        if category_all:
            ...
            if not all_rows:
                ...
                if use_date_filter:
                    ...
                    st.info(f"📅 기간 필터 적용: {date_from} ~ {date_to} → {len(df)}건")
                st.markdown("---")
                ...
                tab1, tab2, tab3, tab4 = st.tabs(
                    ["📋 제품 목록", "📊 품목유형별 분석", "🤖 AI 현황분석", "📥 원시 데이터"]
                )
                with tab1:
                    st.markdown(f"### 📋 {category} 전체 품목 ({len(df)}건)")
                    render_product_table(df, show_type_filter=True, key_prefix="cat_all")
                ...
                with tab3:
                    render_ai_tab(df, category, openai_key)
        ...
        # ── 개별 품목유형 ──
            ...
            if rows is None:
                ...
                tab1, tab2, tab3, tab4 = st.tabs(
                    ["📋 제품 목록", "📊 분석 차트", "🤖 AI 현황분석", "📥 원시 데이터"]
                )

                with tab1:
                    st.markdown(f"### 📋 {food_type} 최신 품목제조보고 ({len(df)}건)")
                    render_product_table(df, show_type_filter=False, key_prefix="single")
                ...
                with tab3:
                    render_ai_tab(df, food_type, openai_key)
    ...
    # ━━━━ 복수 유형 비교 ━━━━
        ...
        if not selected_types:
            ...
            scols = st.columns(min(len(selected_types), 5))
            ...
            if all_rows:
                ...
                tab1, tab2, tab3, tab4 = st.tabs(
                    ["📋 통합 목록", "📊 유형별 비교", "🤖 AI 현황분석", "📥 데이터"]
                )

                with tab1:
                    st.markdown(f"### 📋 통합 품목 목록 ({len(df)}건)")
                    render_product_table(df, show_type_filter=True, key_prefix="multi")
                ...
                with tab3:
                    ...
                    render_ai_tab(df, label, openai_key)
